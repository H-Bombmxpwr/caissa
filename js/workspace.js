...
(function () {
  ...
  function harvestCommands(parsed){
    ...
    for(const node of allNodes(parsed.root)){
      ...
      if(kept.length)node.commands=[...new Set((node.commands||[]).concat(kept))];node.comment=text||null;
    }
  ...
  function filterDialog(reload){modal('Find the games that matter',(body,close)=>{
    ...
    body.append(h('p.muted',{text:'Every field narrows the same search. Colour and outcome are read from the player you name, and choosing an opening fills in the ECO range it covers in your library.'}),
      ...
      field('Study category',inputs.category=select([['','Any category'],...LibraryTools.categories],f.category||'')),
  ...
  const PANELS=[['notation','Notation'],['engine','Stockfish · local analysis'],['tags','Game tags'],
    ['context','Position context'],['tablebase','Endgame tablebase'],['books','Books'],['openingbook','Opening book']];
  const DEFAULT_LAYOUT={main:['notation','engine','tags'],side:['context','tablebase','books','openingbook'],hidden:['books','openingbook']};
  let boardSeq=0;
  function normalizeLayout(saved){
    const source=saved&&typeof saved==='object'?saved:DEFAULT_LAYOUT;
    const layout={main:[],side:[],hidden:[],wide:[...(source.wide||[])].filter(id=>PANELS.some(p=>p[0]===id)),heights:{...(source.heights||{})},cols:{...(source.cols||{})}},placed=new Set();
  ...
  async function analysis() {
    ...
    const comment=h('textarea',{rows:3,placeholder:'What is the idea in this position?','aria-label':'Position comment',
      oninput:()=>{const target=state.node,text=comment.value.trim();if((target.comment||'')===text)return;target.comment=text||null;markDirty();renderMoves();},onblur:()=>{harvestCommands(parsed);comment.value=state.node.comment||'';renderMoves();board.setShapes(state.node.shapes||[]);}});
    ...
    function panelControls(id){
      const where=layout.main.includes(id)?'main':'side',list=layout.wide.includes(id)?layout.wide:layout[where],at=list.indexOf(id),title=built[id].title;
      const step=delta=>{const to=at+delta;if(to<0||to>=list.length)return;list.splice(at,1);list.splice(to,0,id);applyLayout();};
      return h('div.toolbar.panel-controls',[h('button.panel-btn',{type:'button',text:layout.wide.includes(id)?'↙':'↔',title:'Toggle full width','aria-label':'Span '+title+' across both columns',onclick:()=>{layout.wide=layout.wide.includes(id)?layout.wide.filter(x=>x!==id):[...layout.wide,id];applyLayout();}}),
        ...
        h('button.panel-btn',{type:'button',text:where==='main'?'▶':'◀',title:where==='main'?'Send to the right column':'Send to the left column','aria-label':'Move '+title+' to the other column',
          onclick:()=>{layout.wide=layout.wide.filter(x=>x!==id);layout[where]=layout[where].filter(x=>x!==id);layout[where==='main'?'side':'main'].push(id);applyLayout();}}),
    ...
    function applyLayout(){if(!layout.hidden.includes('openingbook'))openingBook.refresh();else openingBook.clearArrows();dockWide.replaceChildren(...layout.wide.filter(id=>!layout.hidden.includes(id)).map(id=>built[id].node));
      ...
      columns.dataset.columns=dockMain.children.length&&dockSide.children.length?'both':dockMain.children.length?'main':'side';
    }
    ...
    function jump(n){if(n===state.node)return;state.node=n;branchIndex=0;board.setShapes(n.shapes||[]);render();moves.querySelector('button.current')?.scrollIntoView({block:'nearest'});}
    ...
    function moveMenu(node){
      const fields=node.parent.fenAfter.split(' '),label=fields[5]+(fields[1]==='w'?'. ':'… ')+node.san;
      let variation=node;
      while(variation.parent&&variation.parent.children[0]===variation)variation=variation.parent;
      modal('Edit '+label,(body,close)=>{
        body.append(h('p',{text:'Deletion includes nested variations and their notes. Other branches are kept. Save the game to write the change to your library.'}));
        function remove(nodes,parent){
          const removed=new Set(nodes.flatMap(allNodes));
          parent.children=parent.children.filter(n=>!nodes.includes(n));
          if(removed.has(state.node))state.node=parent;
          close();markDirty();render();
        }
        body.append(button('Delete move and continuation ('+allNodes(node).length+' moves)',()=>remove([node],node.parent),'danger'));
        if(node.children.length){const count=node.children.reduce((total,n)=>total+allNodes(n).length,0);
          body.append(button('Delete following moves only ('+count+' moves)',()=>remove([...node.children],node),'danger'));}
        if(variation.parent)body.append(button('Delete entire variation ('+allNodes(variation).length+' moves)',()=>remove([variation],variation.parent),'danger'));
    ...
    async function renderFacts(id){
      contextBody.replaceChildren(LibraryTools.localFacts(parsed),h('p.muted',{text:'Looking this game up\u2026'}));
      ...
      try{
        ...
        contextBody.replaceChildren(LibraryTools.localFacts(parsed),h('div.eyebrow',{text:'Background from Wikipedia'}));
      ...
      }catch(err){if(id===contextRequest)contextBody.replaceChildren(LibraryTools.localFacts(parsed),h('p.error-message',{text:err.message}));}
    }
    ...
    async function renderContext(){const id=++contextRequest,position=state.node.fenAfter;contextTabs.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.textContent===state.context));if(state.context==='Facts')return renderFacts(id);contextBody.replaceChildren(h('p.muted',{text:'Finding connections…'}));try{const data=await api('study/position?'+new URLSearchParams({fen:position}));if(id!==contextRequest||state.view!=='analysis')return;contextBody.replaceChildren();
      ...
      if(state.context==='History'&&data.games.length){contextBody.append(h('h3',{text:'Games reaching this position'}),h('p.muted',{text:'Up to 100 games, oldest first. Open any game to compare its continuation.'}),...data.games.map(g=>h('div.history-game',[h('span',{text:g.date||'Undated'}),button(g.white+' — '+g.black,()=>openGame(g.id)),h('small',{text:[g.event,g.result,g.opening].filter(Boolean).join(' · ')})])));}
    ...
    async function annotate(){harvestCommands(parsed);const annotationNodes=[parsed.root,...PGN.mainline(parsed.root)];live=false;liveBox.checked=false;clearTimeout(liveTimer);clearTimeout(livePoll);++evalRequest;await api('engine/live',null,'DELETE');await api('engine/annotate',{game_id:state.selected?.id,positions:annotationNodes.map(n=>({ply:n.ply,fen:n.fenAfter,san:n.san})),movetime:200});clearInterval(poll);poll=setInterval(act(async()=>{const status=await api('engine/annotate');engineBody.replaceChildren(h('p.muted',{style:{padding:'16px'},text:`Annotating ${status.done} / ${status.total} positions…`}),button('Stop annotation',()=>api('engine/stop',{})));if(!status.running){clearInterval(poll);if(status.error)throw new Error(status.error);const nodes=annotationNodes;status.results.forEach((r,i)=>{const n=nodes[i];if(!n)return;const mate=r.mate==null?null:r.mate*(n.fenAfter.split(' ')[1]==='w'?1:-1);const value=mate!=null?'#'+mate:r.cp!=null?(r.cp/100).toFixed(2):null;if(value!==null){n.eval=mate!=null?{mate}:{cp:r.cp};n.commands=(n.commands||[]).filter(c=>!/^\[%eval\b/i.test(c));n.commands.push('[%eval '+value+']');}if(r.played_judgment&&nodes[i+1]){const glyph={'blunder':'$4','mistake':'$2','inaccuracy':'$6'}[r.played_judgment];if(glyph&&!nodes[i+1].nags.length)nodes[i+1].nags=[glyph];}});markDirty();render();engineBody.replaceChildren(h('p.muted',{style:{padding:'16px'},text:'Annotation complete. Save changes to keep these evaluations in your PGN.'}));}}),700);}
    ...
    render();renderTags();
    const key=e=>{if(state.view!=='analysis'||/INPUT|TEXTAREA|SELECT/.test(e.target.tagName)||e.target.isContentEditable||document.querySelector('dialog[open]')||e.ctrlKey||e.altKey||e.metaKey)return;
      let next;if(e.key==='ArrowLeft')next=state.node.parent||state.node;if(e.key==='ArrowRight')next=state.node.children[branchIndex]||state.node;
  ...
  function viewRepertoire(rep){
    ...
    if(!groups.size)throw new Error('This repertoire has no lines yet.');
    stashBoard();
  ...
  function drillLine(nodes,fen,rep,line,sessionLines){
    ...
    modal('Practise '+(rep?.name||'this line'),(body,close)=>{
      ...
      const named=h('p.drill-opening',{'aria-live':'polite'});
      const owner=rep?.color||new Chess(fen).turnColor();
      const openings=new Map();   // line key -> the plies at which its opening is renamed
      let index=0,mistakes=0,hints=0,stage=0,g=new Chess(fen),completed=false,waiting=false,saved=false,timer;
      ...
      /* The opening is named for the position actually reached, so it follows the player
         into the line without telling them where the line is going. */
      function showOpening(){
        const stages=openings.get(key(currentLine));
        if(!stages)return;
        const reached=stages.filter(entry=>entry.ply<=index).pop();
        named.textContent=reached?reached.eco+' · '+reached.opening:'';
      }
      async function loadOpening(){
        const id=key(currentLine);
        named.textContent=openings.has(id)?named.textContent:'';
        if(!openings.has(id)){
          const moves=currentLine.moves.map(move=>typeof move==='string'?move:move.san);
          openings.set(id,(await api('openings/name',{moves,fen:currentLine.fen||null})).names||[]);
          if(key(currentLine)!==id)return;   // the player branched while this was in flight
        }
        showOpening();
      }
      function record(san){
        ...
        b.setLastMove([move.from,move.to]);showOpening();
      }
      function submit(move){
        ...
        if(played&&played.san!==sans[index]){
          ...
          if(alternative>=0){const next=queue.splice(alternative,1,currentLine)[0];currentLine=next;sans=next.moves;loadOpening();}
        ...
        record(played.san);clearHint();feedback.textContent='Correct.';waiting=true;
      ...
      function renderBoard(){
        ...
        hint.disabled=completed||waiting;
      }
      function clearHint(){
        stage=0;b.setHighlights({},{answer:true});hint.textContent='Hint';
      }
      const hint=button('Hint',()=>{
        if(completed||waiting)throw new Error('There is no move to hint at right now.');
        const test=new Chess(g.fen()),move=test.move(sans[index]);
        if(!move)throw new Error('This saved line contains an illegal move: '+sans[index]);
        if(!stage){
          stage=1;hints++;App.stat('repertoire-hints',{count:1});
          b.setHighlights({[move.from]:'hl-blue'},{answer:true});
          hint.textContent='Show the square';
          feedback.textContent='Move the highlighted piece. Press again for the square it goes to.';
        }else{
          stage=2;b.setHighlights({[move.from]:'hl-blue',[move.to]:'hl-yellow'},{answer:true});
          hint.textContent='Hint shown';hint.disabled=true;
          feedback.textContent='Play '+move.from+'–'+move.to+'. A hinted line comes back tomorrow.';
        }
      });
      const finish=button('Save review',async()=>{
        ...
        if(rep){
          ...
          for(const review of results){
            ...
            if(!item)continue;
            const clean=!review.mistakes&&!review.hints;
            item.successes=(item.successes||0)+(clean?1:0);
            item.interval=clean?Math.max(1,Math.round((item.interval||.4)*2.5)):1;item.due=Date.now()+item.interval*86400000;
          }
          await api('repertoires/'+rep.id,{name:latest.name,color:latest.color,data},'PUT');
        }
        App.stat('repertoire',{reviews:results.length,mistakes:results.reduce((sum,r)=>sum+r.mistakes,0),hints:results.reduce((sum,r)=>sum+r.hints,0)});
      ...
      function nextBranch(){
        ...
        index=0;mistakes=0;hints=0;completed=false;waiting=false;g=new Chess(fen);history.replaceChildren();b.setLastMove(null);
        clearHint();loadOpening();
      ...
      function advance(){
        ...
        if(completed){
          results.push({line:currentLine,mistakes,hints});
          if(queue.length){
      ...
      body.append(h('div.repertoire-drill-layout',[
        ...
        h('div.drill-sidebar',[h('p',{text:'You play '+(owner==='w'?'White':'Black')+'. Drag pieces or click a piece and its destination. Opponent moves play automatically.'}),
          progress,prompt,named,h('h4',{text:'Moves played'}),history,promotion,feedback,h('label.toolbar',[blind,'Blindfold mode (optional)']),h('div.toolbar',[hint,peek]),
  ...
  function collectionRenameDialog(c){modal('Rename collection',(body,close)=>{
    const name=h('input',{value:c.name});
    body.append(field('Collection name',name),
      h('p.muted',{text:'The folder holding this collection’s PGN is renamed with it, so its games stay readable and its study folder manifest is rewritten. A name another collection already uses is refused.'}),
      h('div.dialog-actions',[button('Cancel',close),button('Save name',async()=>{
        if(!name.value.trim())throw new Error('Enter a collection name.');
        await api('collections/'+c.id,{name:name.value.trim()},'PUT');
        close();await refreshMeta();await go('studies');App.toast('Collection renamed');
      },'primary')]));name.focus();name.select();
  });}
  ...
  async function imports(){await importHistory();content.append(heading('Bring your chess home','Import games','PGN files, master collections, and your own online games — together in one library.'));const collection=h('input',{placeholder:'e.g. Tournament games'}),status=h('div.status-message'),pgn=h('textarea',{rows:9,placeholder:'Paste one game or an entire collection in PGN format…'}),files=h('input',{type:'file',accept:'.pgn',multiple:true});
    ...
    // The server already falls back to the token stored in Settings, so a connected
    // account should never be asked to paste one again.
    const tokenField=field('lichess token',token),tokenNote=h('p.account-note');
    const showToken=CaissaAccount.watch(tokenField,data=>{
      const linked=data.connected&&source.value==='lichess';
      tokenField.hidden=linked;tokenNote.hidden=!linked;
      if(linked){token.value='';tokenNote.replaceChildren(h('b',{text:'lichess connected'}),
        h('span',{text:' · importing as '+data.username+'. The token saved in Settings is used, so there is nothing to paste.'}));}
    });
    source.addEventListener('change',showToken);
  ...
  async function lichessAccount(){
    ...
    const badge=h('p.account-badge',{'aria-live':'polite'});
    ...
    const tokenField=field('Personal access token',token);
    const actions=h('div.toolbar');
    const watcher=h('div');
    const body=h('div.card-pad',[
      badge,
      h('p.muted',{text:'Connect your lichess account to import your own games and your private or unlisted studies. The token is stored in your local library and is never sent anywhere but lichess. Once it is connected, your username fills itself in across Caissa and no import asks for the token again.'}),
      detail,tokenField,actions,status,watcher]);
    async function refresh(){
      const data=await api('lichess/account');CaissaAccount.set(data);
      actions.replaceChildren();
      badge.dataset.state=data.connected?'connected':'off';
      badge.textContent=data.connected?'Connected as '+(data.title?data.title+' ':'')+data.username:'Not connected';
      tokenField.hidden=false;
      if(data.connected){
        const scopes=(data.scopes||[]).join(', ')||'no scopes reported';
        detail.replaceChildren(h('span',{text:'Token permissions: '+scopes}));
        ...
        detail.replaceChildren(link('Create a token with the right permissions',data.token_url));
  ...
  async function settings(){content.append(heading('Make Caissa yours','Settings','Choose where your library lives and how your board looks.'));const holder=h('div.board-holder',{style:{maxWidth:'460px',margin:'auto'}});preview=new Board(holder,{viewOnly:true});preview.setPosition(new Chess());
    ...
    content.append(h('div.settings-grid',[card('Board & pieces',h('div.card-pad',[field('Board palette',swatches),h('div.toolbar',[field('Light squares',light),field('Dark squares',dark)]),field('Dark theme',h('input',{type:'checkbox',checked:!!state.prefs.darkMode,onchange:act(async e=>{state.prefs.darkMode=e.target.checked;await savePrefs();})})),field('Piece set',pieceSet),field('Piece treatment',treatments),field('Default orientation',orientation),field('Coordinates',coordinates),field('Animate moves and captures',animate),field('Animation duration',select([['100','Fast · 100 ms'],['200','Normal · 200 ms'],['350','Smooth · 350 ms'],['500','Slow · 500 ms']],String(state.prefs.animationMs||200),act(async e=>{state.prefs.animationMs=Number(e.target.value);await savePrefs();})))])),card('Preview',h('div.card-pad',[holder,h('p.muted',{style:{marginTop:'20px'},text:'Your palette applies to analysis, previews, and all seven blindfold exercises.'})]))]));
    const sections=[...content.children].slice(1),tabs=h('div.section-tabs',{'aria-label':'Settings sections'}),pane=h('div.settings-pane');
    const groups=[['Appearance',[sections.at(-1)]],['Sound',[sections[2]]],['Account & connection',sections.slice(0,2)],['Library & storage',sections.slice(3,-1)]];
    function show(label,nodes){pane.replaceChildren(...nodes.filter(Boolean));tabs.querySelectorAll('button').forEach(b=>{b.classList.toggle('active',b.dataset.section===label);b.setAttribute('aria-pressed',String(b.dataset.section===label));});}
    groups.forEach(([label,nodes])=>{const tab=button(label,()=>show(label,nodes));tab.dataset.section=label;tabs.append(tab);});
    // The lichess connection lives behind a tab, so the tab itself says whether there is one.
    const accountTab=tabs.querySelector('[data-section="Account & connection"]');
    CaissaAccount.watch(accountTab,data=>{
      accountTab.dataset.account=data.connected?'on':'off';
      accountTab.title=data.connected?'lichess connected as '+data.username:'No lichess account connected';
    });
